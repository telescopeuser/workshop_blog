...
casper.then(function() {casper.start('https://github.com/telescopeuser/workshop_blog/stargazers?page='+page_no+'', function() {
techo('https://github.com/telescopeuser/workshop_blog/stargazers?page='+page_no+'' + ' - ' + this.getTitle() + '\n');});}); // end of dynamic url block
...
casper.then(function() {{techo('snap .public');
casper.waitFor(function check() {return check_tx('.public');},
function then() {this.captureSelector(snap_image(),tx('.public'));},
function timeout() {this.echo('ERROR - cannot find .public').exit();});}});
...
casper.then(function() {dom_result = ''; dom_result = this.evaluate(function(dom_json) {id_list = ""; for (n=1;n<=51;n++) {id_list += document.querySelector('.follow-list-item:nth-child('+n+') a').href + '\n'}; return id_list;}, dom_json);});

casper.then(function() {this.echo(dom_result);});
