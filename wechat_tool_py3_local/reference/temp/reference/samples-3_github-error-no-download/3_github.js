...
var flow_path = '/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-3_github';

casper.start('https://github.com/tebelorg/Tmail', function() {
techo('https://github.com/tebelorg/Tmail' + ' - ' + this.getTitle() + '\n');});
...
casper.then(function() {{techo('read (//*[@class="numbers-summary"]/li)[5] to license_type');
casper.waitFor(function check() {return check_tx('(//*[@class="numbers-summary"]/li)[5]');},
function then() {license_type = this.fetchText(tx('(//*[@class="numbers-summary"]/li)[5]')).trim();},
function timeout() {this.echo('ERROR - cannot find (//*[@class="numbers-summary"]/li)[5]').exit();});}});
...
casper.then(function() {{if ((license_type == "MIT"))
this.echo("Yes, repo is under MIT license.");
else this.echo("No, repo is not under MIT license.");}});
...
casper.then(function() {{techo('click Clone or download');
casper.waitFor(function check() {return check_tx('Clone or download');},
function then() {this.click(tx('Clone or download'));},
function timeout() {this.echo('ERROR - cannot find Clone or download').exit();});}});

casper.then(function() {{techo('click Download ZIP');
casper.waitFor(function check() {return check_tx('Download ZIP');},
function then() {this.click(tx('Download ZIP'));},
function timeout() {this.echo('ERROR - cannot find Download ZIP').exit();});}});
...
casper.then(function() {{techo('api http://services.groupkt.com/country/get/iso2code/'+country+'');
api_result = ''; api_result = call_api('http://services.groupkt.com/country/get/iso2code/'+country+'');
try {api_json = JSON.parse(api_result);} catch(e) {api_json = JSON.parse('null');}}});

casper.then(function() {this.echo(api_result);});

casper.then(function() {this.echo('Country name - '+api_json.RestResponse.result.name);});
