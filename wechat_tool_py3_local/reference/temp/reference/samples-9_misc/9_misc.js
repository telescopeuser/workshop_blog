...
var flow_path = '/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-9_misc';
...
casper.then(function() {{techo('click ABOUT');
casper.waitFor(function check() {return check_tx('ABOUT');},
function then() {this.click(tx('ABOUT'));},
function timeout() {this.echo('ERROR - cannot find ABOUT').exit();});}});
...
casper.then(function() {techo('popup about_tebel');});
casper.waitForPopup(/about_tebel/, function then() {},
function timeout() {this.echo('ERROR - cannot find popup about_tebel').exit();});
casper.withPopup(/about_tebel/, function() {
{ // start of code block

casper.then(function() {{techo('show file-about_tebel-LC2');
casper.waitFor(function check() {return check_tx('file-about_tebel-LC2');},
function then() {this.echo(this.fetchText(tx('file-about_tebel-LC2')).trim());},
function timeout() {this.echo('ERROR - cannot find file-about_tebel-LC2').exit();});}});

casper.then(function() {{techo('snap file-about_tebel-LC2');
casper.waitFor(function check() {return check_tx('file-about_tebel-LC2');},
function then() {this.captureSelector(snap_image(),tx('file-about_tebel-LC2'));},
function timeout() {this.echo('ERROR - cannot find file-about_tebel-LC2').exit();});}});
...
casper.then(function() {this.echo('returning to '+dummy_variable+' window context');});

// after the steps in popup context are completed, context returns to original window
casper.then(function() {{techo('show (//*[@id="header"]//a)[1]');
casper.waitFor(function check() {return check_tx('(//*[@id="header"]//a)[1]');},
function then() {this.echo(this.fetchText(tx('(//*[@id="header"]//a)[1]')).trim());},
function timeout() {this.echo('ERROR - cannot find (//*[@id="header"]//a)[1]').exit();});}});
...
casper.then(function() {dom_result = ''; dom_result = this.evaluate(function(dom_json) {document.querySelector('#header p').focus()}, dom_json);});
...
casper.then(function() {dom_result = ''; dom_result = this.evaluate(function(dom_json) {return document.querySelector('#header p').innerText}, dom_json);});

casper.then(function() {this.echo('dom_result from 1 JavaScript statement - '+dom_result);});

// multiple lines can be combined into a single statement before returning value
casper.then(function() {dom_result = ''; dom_result = this.evaluate(function(dom_json) {document.querySelector('#header p').blur(); return document.querySelector('#header p').innerText}, dom_json);});

casper.then(function() {this.echo('dom_result from multiple statements - '+dom_result);});
...
casper.then(function() {dom_result = ''; dom_result = this.evaluate(function(dom_json) {return document.querySelector(dom_json).innerText}, dom_json);});

casper.then(function() {this.echo('dom_result using 1 dom_json variable - '+dom_result);});
...
casper.then(function() {dom_result = ''; dom_result = this.evaluate(function(dom_json) {return document.querySelector(dom_json.element1).innerText}, dom_json);});

casper.then(function() {this.echo('dom_result using multiple dom_json variables - '+dom_result);});

casper.then(function() {dom_result = ''; dom_result = this.evaluate(function(dom_json) {return document.querySelector(dom_json.element2).innerText}, dom_json);});
...
casper.then(function() {this.echo('test_number - '+test_number);});
...
casper.thenOpen('https://github.com/tebelorg/TLE', function() {
techo('https://github.com/tebelorg/TLE' + ' - ' + this.getTitle());});
...
casper.then(function() {this.echo('test_string - '+test_string);});

// for loop can be expressed in natural language or JavaScript
casper.then(function() {for (column=1; column<=6; column++)
{casper.then(function() {for_loop_signal = '[CONTINUE_SIGNAL][column]';});
(function (column) { // start of IIFE pattern
{ // start of code block

// '+variable_name+' can be used where text is expected
casper.then(function() {{techo('show (//table)[3]//td['+column+']');
casper.waitFor(function check() {return check_tx('(//table)[3]//td['+column+']');},
function then() {this.echo(this.fetchText(tx('(//table)[3]//td['+column+']')).trim());},
function timeout() {this.echo('ERROR - cannot find (//table)[3]//td['+column+']').exit();});}});
...
})(column); // end of IIFE pattern, with dummy marker for break step
casper.then(function() {for_loop_signal = '[BREAK_SIGNAL][column]';});}});
