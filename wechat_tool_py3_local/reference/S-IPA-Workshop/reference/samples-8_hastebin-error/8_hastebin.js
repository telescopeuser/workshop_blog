...
var flow_path = '/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-8_hastebin';

casper.start('https://hastebin.com', function() {
techo('https://hastebin.com' + ' - ' + this.getTitle() + '\n');});
...
if (p2) p1 = p1 + ' ' + p2; if (p3) p1 = p1 + ' ' + p3; if (p4) p1 = p1 + ' ' + p4;
if (p5) p1 = p1 + ' ' + p5; if (p6) p1 = p1 + ' ' + p6; // done for readability
}); // end of JS code
...
casper.then(function() {{techo('type //textarea as '+combined_data+'');
casper.waitFor(function check() {return check_tx('//textarea');},
function then() {this.sendKeys(tx('//textarea'),''+combined_data+'');},
function timeout() {this.echo('ERROR - cannot find //textarea').exit();});}});

casper.then(function() {{techo('click save function button-picture enabled');
casper.waitFor(function check() {return check_tx('save function button-picture enabled');},
function then() {this.click(tx('save function button-picture enabled'));},
function timeout() {this.echo('ERROR - cannot find save function button-picture enabled').exit();});}});

casper.then(function() {{techo('click raw function button-picture enabled');
casper.waitFor(function check() {return check_tx('raw function button-picture enabled');},
function then() {this.click(tx('raw function button-picture enabled'));},
function timeout() {this.echo('ERROR - cannot find raw function button-picture enabled').exit();});}});

casper.then(function() {this.echo('UPLOADED URL - '+this.getCurrentUrl()+'\n');});
