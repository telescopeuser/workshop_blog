...
var flow_path = '/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-7_testing';

casper.start('https://ca.finance.yahoo.com', function() {
techo('https://ca.finance.yahoo.com' + ' - ' + this.getTitle() + '\n');});
...
casper.then(function() {{techo('read search-buttons to search_button_text');
casper.waitFor(function check() {return check_tx('search-buttons');},
function then() {search_button_text = this.fetchText(tx('search-buttons')).trim();},
function timeout() {this.echo('ERROR - cannot find search-buttons').exit();});}});

casper.then(function() {{if ((search_button_text == "Search"))
this.echo("search button text is correct");
else this.echo("search button text is wrong");}});
...
casper.then(function() {{if ((present('search-buttons')))
this.echo('search button exists');
else this.echo('search button does not exist');}});

// check whether the search box is visible
// something can exists on webpage but set as hidden or not showing in its property
casper.then(function() {{if ((visible('uh-search')))
this.echo('search box is visible');
else this.echo('search box is not visible');}});

// check whether the search button text is correct
// same as earlier example above but with output of button text
casper.then(function() {{techo('read search-buttons to button');
casper.waitFor(function check() {return check_tx('search-buttons');},
function then() {button = this.fetchText(tx('search-buttons')).trim();},
function timeout() {this.echo('ERROR - cannot find search-buttons').exit();});}});

casper.then(function() {{if ((button == 'Search'))
this.echo('search button text is correct - '+button);
else this.echo('search button text is wrong - '+button);}});

// check that the page url has yahoo in it (\' can be used to escape the single quote)
casper.then(function() {{if (((url().indexOf('yahoo')>-1)))
this.echo('url contains \'yahoo\'');
else this.echo('url does not contain \'yahoo\'');}});

// check that the page title has Finance in it (\' can be used to escape the single quote)
casper.then(function() {{if (((title().indexOf('Finance')>-1)))
this.echo('title contains \'Finance\'');
else this.echo('title does not contain \'Finance\'');}});

// check that the page has the text My Portfolio (double quotes can also be used for text)
casper.then(function() {{if (((text().indexOf("My Portfolio")>-1)))
this.echo("page text contains My Portfolio");
else this.echo("page text does not contain My Portfolio");}});
...
casper.then(function() {{if ((count('uh-tb-') >= 6))
this.echo('header menu items >= 6');
else this.echo('header menu items < 6');}});
...
casper.then(function() {this.echo('count_number = '+count_number);});
...
casper.then(function() {{techo('type uh-search as microsoft');
casper.waitFor(function check() {return check_tx('uh-search');},
function then() {this.sendKeys(tx('uh-search'),'microsoft');},
function timeout() {this.echo('ERROR - cannot find uh-search').exit();});}});

casper.then(function() {{techo('click //*[@id="search-button"]');
casper.waitFor(function check() {return check_tx('//*[@id="search-button"]');},
function then() {this.click(tx('//*[@id="search-button"]'));},
function timeout() {this.echo('ERROR - cannot find //*[@id="search-button"]').exit();});}});
...
casper.then(function() {this.echo('time taken - '+time_taken+' seconds');});

// by using timer(), the time taken for some actions can also be used as testing criteria
casper.then(function() {{if ((time_taken < 60))
this.echo('step takes lesser than 60 seconds');
else this.echo('step takes more than 60 seconds');}});
...
casper.then(function() {techo('wait 13.57 seconds');});
casper.wait(13570, function() {});

// after the delay, negative test case to check that the quote is APPLE
casper.then(function() {{techo('read //*[@id="quote-header-info"]//h1 to stock_quote');
casper.waitFor(function check() {return check_tx('//*[@id="quote-header-info"]//h1');},
function then() {stock_quote = this.fetchText(tx('//*[@id="quote-header-info"]//h1')).trim();},
function timeout() {this.echo('ERROR - cannot find //*[@id="quote-header-info"]//h1').exit();});}});

casper.then(function() {this.echo('intentional negative test case, to see if quote returned contains APPLE');});

casper.then(function() {{if (((stock_quote.indexOf("APPLE")>-1)))
this.echo('quote is correct - '+stock_quote);
else this.echo('quote is wrong - '+stock_quote);}});
